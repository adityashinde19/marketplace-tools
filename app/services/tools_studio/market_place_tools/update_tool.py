...
class GitHubUpdateWorkflowManager:
    def __init__(self):
    ...
    def trigger_update_workflow(self, app_name: str, unique_id: str, config_type: str, key_name: str, key_value: str, is_secret: bool) -> None:
        """Triggers the workflow and returns immediately"""
        response = requests.post(
            f"https://api.github.com/repos/{self.github_repo}/actions/workflows/update.yml/dispatches",
            headers={
    ...
    def get_workflow_run_by_artifact(self, unique_id: str, timeout: int = 300) -> int:
        ...
        while time.time() - start_time < timeout:
            runs = requests.get(
                f"https://api.github.com/repos/{self.github_repo}/actions/workflows/update.yml/runs",
                headers={
                    "Authorization": f"token {self.github_token}",
                    "Accept": "application/vnd.github+json"
                },
