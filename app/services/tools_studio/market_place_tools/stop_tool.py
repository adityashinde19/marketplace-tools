...
class GitHubStopWorkflowManager:
    def __init__(self):
    ...
    def trigger_stop_workflow(self, app_name: str, unique_id: str) -> None:
        """Triggers the workflow and returns immediately"""
        response = requests.post(
            f"https://api.github.com/repos/{self.github_repo}/actions/workflows/trialstop.yml/dispatches",
            headers={
    ...
    def get_workflow_run_by_artifact(self, unique_id: str, timeout: int = 300) -> int:
        ...
        while time.time() - start_time < timeout:
            runs = requests.get(
                f"https://api.github.com/repos/{self.github_repo}/actions/workflows/trialstop.yml/runs",
                headers={
                    "Authorization": f"token {self.github_token}",
                    "Accept": "application/vnd.github+json"
                },
